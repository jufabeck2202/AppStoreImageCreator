...
func loadImages(path string, info os.FileInfo, images *[]image.Image) {
	if !info.IsDir() {
		fimg, _ := os.Open(path)
		defer fimg.Close()
...
func loadImageChannels(path string, info os.FileInfo, e error, images chan image.Image, errors chan error) {
	if e != nil {
		errors <- e
		return
	}
	...
	if len(os.Args) != 6 {
		...
		if errNr == nil && errDw == nil && errDh == nil && (imageShape == RectangleShape || imageShape == CircleShape) {
			...
			_ = filepath.Walk(dirName, func(path string, info os.FileInfo, e error) error {
				go loadImageChannels(path, info, e, imagesChannel, errChannel)
				return nil
			})
